...
[GitHub Pages](https://iai-ustc-quantum.github.io/QRAM-Simulator/)）。站点为
**中英双语（默认英文，提供中文）**，同时承载：

- **使用指南**（安装 / 快速上手 / 架构 / 算子约束 / 命名规范）——Markdown（MyST）
- **C++ API 参考**——由 Doxygen 头文件注释经 **breathe** 吸入 Sphinx
...
docs/
├── sphinx/
│   ├── Makefile               # make html（双树构建）
│   ├── requirements.txt       # Sphinx 构建依赖
│   └── source/
│       ├── _conf_base.py      # 共享 Sphinx 配置（主题、breathe、autoapi）
│       ├── _shared/           # 共享静态文件与模板（语言切换器）
│       ├── api_stubs/         # CI 注入的 .pyi 桩（gitignored）
│       ├── en/                # 英文树（默认语言）
│       │   ├── conf.py、index.rst
...
> 章节（`_conf_base.py` 检测 `api_stubs/` 是否存在）。

## CI（docs.yml）

push/PR 触发 → doxygen → pip install . + stubgen → sphinx-build（zh + en）→
上传 artifact；main 分支 push 时部署 gh-pages（全量替换；根 URL 重定向到
英文树）。
